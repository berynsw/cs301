...
;recursive funcs for grabbing everything that follows or precedes y
(define (yRz y list [result '()])
  (cond
    [(equal? list null) null]
    [(equal? (car (car list)) y) (yRz y (cdr list) (cons (cdr (car list)) result))]
    [else (yRz y (cdr list) result)]))

(define (xRy y list [result '()])
  (cond
    [(equal? list null) result]
    [(equal? (cdr (car list)) y) (xRy y (cdr list) (cons (car (car list)) result))]
    [else (xRy y (cdr list) result)]))

...
; for-all
;
; for-all is given a list A and a lambda function fx. At each step, it gets the
; first value of A (car A) and applies to fx. If fx returns true, it recursively
; calls for-all with the remainder of the list (cdr A), otherwise returns false.
;
; If A is null, returns true.
;
; Parameters:
;  A (list)
...
; there-exists
;
; there-exists is given a list A and a lambda function fx. At each step, it gets the
; first value of A (car A) and applies it to fx. If fx returns true, there-exists
; immediately returns true. Otherwise recursively calls there-exists with the remainder
; of A (cdr A).
;
; If A is null, returns false.
;
; Parameters:
;  A (list)
;  fx (function): inserted lambda function to check properties in list ex: does list contain the reverse of a given pair?
...
; predicate-logic
;
; At each given step, if L is not null, it pops off the next quantifier
; ('forall or 'exists) and recursively calls for-all or there-exists.
; The nested for-all and there-exists will use a lambda function that will
; call predicate-logic again with the same A, the rest of L (the cdr of L),
; same fx, and the running curry function;
;
; The curry function is by default created using the fx. Fx is not used anywhere else.
; At each step, the curry gets closer to completing and eventually return its result.
; for-all and there-exists are responsible for adding these variables to the curry.
;
; Parameters:
;  A (list)
;  L (list): a list of quantifiers in a given wff, forall or exists
;  fx (function): inserted lambda function to check properties in list ex: if x is a tree is it a plant?
;  curryFx (function): a curry function - takes in parameters until the default function is satisfied and returns a value instead of a procedure [OPTIONAL, default (curry fx)]
;
; Returns:
;  true or false, the result of the curry function
